...
import { buildSearchSuggestions, normalizeSearchText } from "../utils/search";
  ...
  async function load() {
    try {
      const [vehicles, contactsResponse] = await Promise.all([
        api.get("/workshop/vehicles/", { params: search ? { search } : {} }),
        api.get("/contacts/"),
      ]);
          ...
          <Row className="g-2 align-items-end">
            <Col md={10}>
              ...
              <SearchAutocompleteInput
                placeholder="Buscar por placa, cliente, marca, modelo ou ano"
                value={search}
                onChange={setSearch}
                onSearch={load}
                suggestions={buildSearchSuggestions(items, ["plate", "customer_name", "make", "model", "version", "year", (vehicle) => [vehicle.make, vehicle.model, vehicle.version].filter(Boolean).join(" ")])}
              />
            </Col>
            <Col md={2}>
