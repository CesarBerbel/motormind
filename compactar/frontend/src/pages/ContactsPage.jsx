            ...
            <TabPanel activeKey={activeTab} eventKey="identification">
              <Row className="g-3">
                ...
                <Col md={4}>
                  ...
                  <div className="date-placeholder-shell">
                    <Form.Control
                      type="date"
                      value={form.birth_date || ""}
                      onChange={(e) => update({ birth_date: e.target.value })}
                      placeholder={dateInputValue()}
                      className={!form.birth_date ? "date-input-empty" : ""}
                    />
                    {!form.birth_date ? <span className="date-placeholder-overlay">{formatDate(dateInputValue())}</span> : null}
                  </div>
                  {form.birth_date ? (
                    <div className="small text-muted mt-1">{formatDate(form.birth_date)}</div>
                  ) : (
                    <div className="small text-muted mt-1">Opcional. Se não preencher, será salvo em branco.</div>
                  )}
                </Col>
                {isCompany && (
                  <>
